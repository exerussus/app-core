...
namespace Exerussus.AppCore.Navigation
{
    ...
    // Сравнение — по id (один long-компэр). default(PageId) == "пустой" (Id 0).
    ...
    public readonly struct PageId : IEquatable<PageId>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private PageId(long id) => Id = id;

        public static readonly PageId None = default;   // Id == 0

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static PageId FromRaw(long id)
        {
            // Отрицательное значение не может быть валидным PageId → сломанные данные.
            if (id < 0)
                Debug.LogError($"[PageId] broken id: отрицательное значение 0x{id:X16} — валидный PageId всегда положителен (0 = None). Перепутано с Popup Uid?");
            return new(id);
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Equals(PageId other) => Id == other.Id;

        public override bool Equals(object obj) => obj is PageId other && Id == other.Id;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public override int GetHashCode() => Id.GetHashCode();

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool operator ==(PageId a, PageId b) => a.Id == b.Id;
        ...
        public override string ToString()
        {
            ...
            return Names.TryGetValue(Id, out var name)
                ? $"PageId(\"{name}\", 0x{Id:X16})"
                : $"PageId(0x{Id:X16})";
#else
            return $"PageId(0x{Id:X16})";
#endif
        }
        ...
        private static void RegisterName(long id, string name)
        {
            if (Names.TryGetValue(id, out var existing))
            {
                if (!string.Equals(existing, name, StringComparison.Ordinal))
                    Debug.LogWarning($"[PageId] id collision: \"{existing}\" и \"{name}\" → 0x{id:X16}");
                return;
            }
            Names[id] = name;
        }
